...
{ 
    public class ObjectRecognizer
    {
        ...
        public ObjectRecognizer(DirectoryInfo directory, string imageDirectoryName, string libraryDirectoryName, IViewResult ResultInterface)
        {
            ...
            imageDirectory = new DirectoryInfo(directory.FullName + "\\" + imageDirectoryName);
            libraryDirectory = new DirectoryInfo(directory.FullName + "\\" + libraryDirectoryName);
        ...
        public ObjectRecognizer(string directory, IViewResult ResultInterface)
        {
            resultInterface = ResultInterface;
        ...
        {
            //Работаем пока есть имя файла в очереди
            while (fileNamesQueue.TryDequeue(out string name))
            {
                //Проверка на отмену
        ...
        Result Recognize(string name)
        {
            using var image = Image.Load<Rgb24>(name);
            ...
            using var session = new InferenceSession(libraryDirectory.FullName + "\\" + "shufflenet-v2-10.onnx");
            ...
            // Возвращаем наиболее вероятный результат
            Result tmp = new Result("","",0);
            foreach (var p in softmax.Select((x, i) => new { Label = classLabels[i], Confidence = x }).OrderByDescending(x => x.Confidence).Take(1))
            {
